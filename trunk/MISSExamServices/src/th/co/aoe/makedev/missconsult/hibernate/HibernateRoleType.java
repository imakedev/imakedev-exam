...
public class HibernateRoleType  extends HibernateCommon implements RoleTypeService {
	...
	public List listRoleTypeByRcId(Long rcId) throws DataAccessException {
		// TODO Auto-generated method stub
		//RoleType roleType = null;
		List<th.co.aoe.makedev.missconsult.xstream.RoleType> roles=new ArrayList<th.co.aoe.makedev.missconsult.xstream.RoleType>();
		Session session=sessionAnnotationFactory.getCurrentSession();
	//	Query query=session.createQuery(" select roleType from RoleType roleType where roleType.mmId=:mmId");
		...
		if(map.size()>0){
			for (Iterator iterator = map.keySet().iterator(); iterator.hasNext();) {
				...
				Object obj=query.uniqueResult();
				if(obj!=null){
					th.co.aoe.makedev.missconsult.hibernate.bean.RoleType  type=(th.co.aoe.makedev.missconsult.hibernate.bean.RoleType)obj;
		...
		if(maId!=null && maId.intValue()!=0){
		 ...
		 logger.debug("exclude="+exclude);
			...
			for (th.co.aoe.makedev.missconsult.hibernate.bean.RoleType type : list) {
				th.co.aoe.makedev.missconsult.xstream.RoleType xrole=new th.co.aoe.makedev.missconsult.xstream.RoleType();
