...
public class RoleMappingResource extends BaseResource {
	...
	protected Representation post(Representation entity, Variant variant)
		...
		try {
			in = entity.getStream();
			xstream.processAnnotations(th.co.aoe.makedev.missconsult.xstream.RoleMapping.class);// or xstream.autodetectAnnotations(true); (Auto-detect  Annotations)
			th.co.aoe.makedev.missconsult.xstream.RoleMapping xbpsTerm = new th.co.aoe.makedev.missconsult.xstream.RoleMapping();
			Object ntcCalendarObj = xstream.fromXML(in);
			if (ntcCalendarObj != null) {
				xbpsTerm = (th.co.aoe.makedev.missconsult.xstream.RoleMapping) ntcCalendarObj;
				if (xbpsTerm != null) {
				
					 
					th.co.aoe.makedev.missconsult.hibernate.bean.RoleMapping bpsTerm = new th.co.aoe.makedev.missconsult.hibernate.bean.RoleMapping();
					...
					if (xbpsTerm.getServiceName() != null
						...
						/*else if(serviceName.equals(ServiceConstant.ROLE_MAPPING_ITEMS_DELETE)){
							...
							List<th.co.aoe.makedev.missconsult.xstream.RoleMapping> xntcCalendars = roleMappingService.listRoleMappingByrcId(xbpsTerm.getRcId());
							...
							if (result != null && result.size() == 2) {
								@SuppressWarnings("unchecked")
								java.util.ArrayList<th.co.aoe.makedev.missconsult.hibernate.bean.RoleMapping> ntcCalendars = (java.util.ArrayList<th.co.aoe.makedev.missconsult.hibernate.bean.RoleMapping>) result
										.get(0);
								String faqs_size = (String) result.get(1);
//								 
								VResultMessage vresultMessage = new VResultMessage();

								List<th.co.aoe.makedev.missconsult.xstream.RoleMapping> xntcCalendars = new ArrayList<th.co.aoe.makedev.missconsult.xstream.RoleMapping>();
...
//			 
	...
	private List<th.co.aoe.makedev.missconsult.xstream.RoleMapping> getxRoleMappingObject(
			java.util.ArrayList<th.co.aoe.makedev.missconsult.hibernate.bean.RoleMapping> ntcCalendars) {
		List<th.co.aoe.makedev.missconsult.xstream.RoleMapping> xntcCalendars = new ArrayList<th.co.aoe.makedev.missconsult.xstream.RoleMapping>(
				ntcCalendars.size());
		for (th.co.aoe.makedev.missconsult.hibernate.bean.RoleMapping missManual : ntcCalendars) {
			th.co.aoe.makedev.missconsult.xstream.RoleMapping xmissManual =new th.co.aoe.makedev.missconsult.xstream.RoleMapping ();
	...
	private Representation returnUpdateRecord(Representation entity,th.co.aoe.makedev.missconsult.xstream.RoleMapping xbpsTerm,int updateRecord){
		VResultMessage vresultMessage = new VResultMessage();
		List<th.co.aoe.makedev.missconsult.xstream.RoleMapping> xbpsTerms = new ArrayList<th.co.aoe.makedev.missconsult.xstream.RoleMapping>(1);
